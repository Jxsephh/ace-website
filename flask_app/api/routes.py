...
def hello():
    return Response('Hello World!', http.OK)

# get all events
@mod.route('/events', methods=['GET'])
def events():
    if not current_user.is_authenticated:
        return Response('Authorization required.', status=http.UNAUTHORIZED)

    return Response(dumps(mongo.db.events.find({})), http.FOUND)
...
def get_event(event_id):
    ...
    if db_response:
        return Response(dumps(db_response), status=http.FOUND)
    else:
...
def create_event():
    ...
    try:
        ...
        return Response('Event did not contain all the required fields.', status=http.BAD_REQUEST)
    ...
    if db_result.upserted_id:
        return Response('Successfully created event.', status=http.CREATED)
    else:
        return Response('Event with that name already exists.', status=http.CONFLICT)
...
def close_event(event_id):
    ...
    if db_result.matched_count:
        return Response('Successfully closed event.', status=http.OK)
    else:
...
def reopen_event(event_id):
    ...
    if db_result.matched_count:
        return Response('Successfully reopened event.', status=http.OK)
    else:
        return Response('Event not found.', status=http.NOT_FOUND)
...
def signup(event_id):
    ...
    if not db_result.matched_count:
        ...
        return Response('User already signed up for event.', status=http.CONFLICT)
    else:
        return Response('Successfully signed up for event.', status=http.OK)
